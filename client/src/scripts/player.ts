...
class Player {
  ...
  constructor($1t: OneTagger) {
    ...
    setInterval(() => {
      ...
      if (this.playing) this.position += 150;
    }, 150);
  }
  ...
  async _generateWaveformInner(path: string) {
    ...
    if (
      $1t.info.value.os != "windows" &&
      $1t.settings.value.continuePlayback &&
      this.wasPlaying
    ) {
      this.play();
      this.wasPlaying = false;
  ...
  pause() {
    const $1t = get1t();
    $1t.settings.value.clientSidePlayer
      ? this.audio.pause()
      : $1t.send("playerPause");
    this.playing = false;
    this.wasPlaying = false;
  ...
  loadTrack(path: string) {
    ...
    if ($1t.settings.value.clientSidePlayer) {
      ...
      this.audio = new Audio(
        `${httpUrl()}/audio?path=${encodeURIComponent(path)}`,
      );
    ...
    // Server side
    this._playerLoadPromise = new Promise(
      (res) => (this._playerLoadResolve = res),
    );
