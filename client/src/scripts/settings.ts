...
class Settings {
  ...
  saveATProfile(name: string, config: AutotaggerConfig) {
    ...
    if (i == -1) {
      this.autoTaggerProfiles.push({
        name,
        config: JSON.parse(JSON.stringify(config)),
      });
      return;
    }
    this.autoTaggerProfiles[i] = {
      name,
      config: JSON.parse(JSON.stringify(config)),
    };
  }
...
class AudioFeaturesSettings {
  ...
  static fromJson(data: any): AudioFeaturesSettings {
    let a: AudioFeaturesSettings = Object.assign(
      new AudioFeaturesSettings(),
      data,
    );
    if (data.config) a.config = AudioFeaturesConfig.fromJson(data.config);
    return a;
  }
}
...
class QuickTagSettings {
  ...
  static fromJson(data: any): QuickTagSettings {
    ...
    qt.subgenreTag = qt.subgenreTag
      ? FrameName.fromJson(data.subgenreTag)
      : undefined;
    return qt;
  }
}
