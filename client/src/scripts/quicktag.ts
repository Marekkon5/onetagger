...
class QTMultiTrack {
  ...
  get genres(): string[] {
    if (this.tracks.length == 0) return [];
    return this.tracks[0].genres.filter((g) =>
      this.tracks.every((t) => t.genres.includes(g)),
    );
  }
...
class QTTrack implements QuickTagFile {
  ...
  constructor(data: QuickTagFile, settings: QuickTagSettings) {
    ...
    if (settings.subgenreTag) {
      let subgenres =
        this.tags[
          this.removeAbstractions(settings.subgenreTag.byFormat(this.format))
        ] ?? [];
      this.originalGenres.push(
        ...subgenres.filter((g) => !this.genres.includes(g)),
      );
  ...
  getNote() {
    ...
    let field = this.removeAbstractions(
      this.settings.noteTag.tag.byFormat(this.format),
    );
    ...
    for (let custom of this.settings.custom) {
      if (custom.tag.byFormat(this.format) == field) {
        noteValues = noteValues.filter(
          (v) => !custom.values.map((i) => i.val).includes(v),
        );
      }
    }
  ...
  getMood() {
    let field = this.removeAbstractions(
      this.settings.moodTag.byFormat(this.format),
    );
    ...
    let t =
      this.tags[
        this.removeAbstractions(
          this.settings.energyTag.tag.byFormat(this.format),
        )
      ];
    if (t) {
  ...
  loadCustom() {
    ...
    for (let custom of this.settings.custom) {
      let t =
        this.tags[this.removeAbstractions(custom.tag.byFormat(this.format))] ??
        [];
  ...
  getOutput() {
    ...
    if (this.getEnergy() != this.energy) {
      ...
      if (this.settings.energyTag.type == "rating") {
        ...
        changes.push({
          ...
          value: this.energy
            ? [this.settings.energyTag.symbol.repeat(this.energy)]
            : [],
        });
      }
    }
    ...
    for (let i = 0; i < original.length; i++) {
      ...
      if (
        rawCustom &&
        (rawCustom.length != original[i].length ||
          !rawCustom.every((v, j) => original[i][j] == v))
      ) {
        let field = this.removeAbstractions(
          this.settings.custom[i].tag.byFormat(this.format),
        );
        let values: string[] = [];
        ...
        if (existingIndex == -1) {
          ...
          values = values.filter(
            (v) =>
              v.trim() &&
              !this.settings.custom[i].values.find((t) => t.val == v),
          );
        }
        ...
        while (existingIndex != -1) {
          values = values.concat(
            changes[existingIndex].value
              ...
              // @ts-ignore
              .filter(
                (v) => !this.settings.custom[i].values.find((t) => t.val == v),
              ),
          );
...
function keyColor(key?: string) {
  ...
  // Camelot or OpenKey
  let color =
    KEY_COLORS[CAMELOT_KEYS[key.toUpperCase()]] ||
    KEY_COLORS[OPENKEY_KEYS[key.toLowerCase()]];
  // Normal
  if (!color) {
