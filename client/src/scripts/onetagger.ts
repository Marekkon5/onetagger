...
class OneTagger {
  ...
  private async incomingEvent(json: any) {
    ...
    switch (json.action) {
      ...
      case "defaultCustomPlatformSettings":
        ...
        if (
          !this.config.value.custom["beatport"] ||
          typeof this.config.value.custom["beatport"]["art_resolution"] ===
            "object"
        ) {
        ...
        for (let platform of Object.keys(json.custom)) {
          ...
          for (let key of Object.keys(json.custom[platform])) {
            if (!this.config.value.custom[platform][key]) {
              this.config.value.custom[platform][key] =
                json.custom[platform][key];
            }
          }
        }
      ...
      case "taggingProgress":
        ...
        // De duplicate failed
        var i = this.taggerStatus.value.statuses.findIndex(
          (s) => s[0].status.path == json.status.status.path,
        );
        if (i != -1) {
      ...
      case "quickTagLoad":
        this.lock.value.locked = false;
        this.quickTag.value.tracks = json.data.files.map(
          (t: QuickTagFile) => new QTTrack(t, this.settings.value.quickTag),
        );
      ...
      /*eslint-disable no-case-declarations*/
      case "quickTagSaved":
        var i = this.quickTag.value.tracks.findIndex(
          (t) => t.path == json.path,
        );
        if (i != -1) {
          this.quickTag.value.tracks[i] = new QTTrack(
            json.file,
            this.settings.value.quickTag,
          );
        } else {
      ...
      case "configCallback":
        ...
        switch (response.type) {
          ...
          case "updateConfig":
            Object.assign(
              this.config.value.custom[json.platform],
              response.config,
            );
            break;
        }
      ...
      case "installPlatform":
        ...
        Dialog.create({
          ...
          message: isOk
            ? "Platform was installed succesfully, press OK to continue"
            : json.error,
          persistent: true,
          cancel: false,
          ok: {
  ...
  browse(context: string, path?: string) {
    if (this.settings.value.nonNativeBrowser) {
      ...
      if (path) this.folderBrowser.value.basePath = path;
  ...
  onBrowse(json: BrowseEvent) {
    // Autotagger path
    if (json.context == "at") this.config.value.path = json.path;
    // Quicktag path
    if (json.context == "qt") {
      this.settings.value.path = json.path;
    ...
    // Audio features path
    if (json.context == "af") this.onAudioFeaturesEvent(json);
    // Tag editor
    if (json.context == "te") this.onTagEditorEvent(json);
    // Renamer
  ...
  saveSettings(notif = true) {
    this.settings.value.saveATProfile(
      this.settings.value.autoTaggerProfile,
      this.config.value,
    );
    // Very dirty way to clone a dict, but eh
    this.settings.value.autoTaggerConfig = JSON.parse(
      JSON.stringify(this.config.value),
    );
    this.settings.value.volume = this.player.value.volume;
    // Save
    this.send("saveSettings", {
      settings: JSON.parse(JSON.stringify(this.settings.value)),
    });
    // Notification
    if (notif)
      Notify.create({
  ...
  loadSettings(data: any) {
    ...
    if (!this.settings.value.tagEditorCustom)
      this.settings.value.tagEditorCustom = [];
    this.spotify.value.clientId =
      this.settings.value.audioFeatures.spotifyClientId;
    this.spotify.value.clientSecret =
      this.settings.value.audioFeatures.spotifyClientSecret;
  ...
  loadATProfile(name: string) {
    let profile = this.settings.value.autoTaggerProfiles.find(
      (p) => p.name == name,
    );
  ...
  loadQTTrack(track?: QTTrack, force = false) {
    ...
    if (force || !this.quickTag.value.track.isChanged()) {
      if (!track && !this.nextQTTrack) return;
      if (!track)
        track = this.quickTag.value.tracks.find(
          (t) => t.path == this.nextQTTrack!.path,
        );

      // For autoplay
      if (this.player.value.playing) this.player.value.wasPlaying = true;

      this.quickTag.value.track.loadSingle(
        new QTTrack(
  ...
  addQTTrack(track: QTTrack) {
    this.quickTag.value.track.addTrack(
      new QTTrack(
        JSON.parse(JSON.stringify(track)),
        this.settings.value.quickTag,
      ),
    ...
    if (this.quickTag.value.track.hasTracks()) {
      ...
      if (event.key.startsWith("Arrow")) {
        ...
        if (event.key == "ArrowLeft") {
          ...
          if (pos < 0) this.player.value.seek(0);
          else this.player.value.seek(pos);
        }
        // Seek forward
        ...
        if (event.key == "ArrowUp") {
          this.onQuickTagEvent(event.shiftKey ? "addTrack" : "changeTrack", {
            offset: -1,
          });
        }
      ...
      // Play pause
      if (event.code == "Space") {
        if (this.player.value.playing) this.player.value.pause();
        else this.player.value.play();
        return true;
      }
      ...
      this.settings.value.quickTag.custom.forEach((tag, tagIndex) => {
        for (let i = 0; i < tag.values.length; i++) {
          if (Keybind.check(event, tag.values[i].keybind)) {
            this.quickTag.value.track!.toggleCustom(
              tagIndex,
              tag.values[i].val,
            );
          }
      ...
      // Delete tracks
      if (
        (event.code == "Delete" && (event.ctrlKey || event.metaKey)) ||
        (event.code == "Backspace" && (event.ctrlKey || event.metaKey))
      ) {
