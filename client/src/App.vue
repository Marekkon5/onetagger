  ...
  <div>
    <q-layout view="hHh lpR fFf" class="bg-background">
        ...
        <q-toolbar class="row justify-between">
          <div class="col-2 row" style="height: 58px">
            <img
              ...
              @click="home"
              class="logo q-mt-sm"
              :class="{ spin: $1t.lock.value.locked }"
            />
      ...
      <q-footer reveal class="bg-darker text-white" v-if="footer">
        ...
        <PlayerBar
          v-if="
            ($1t.settings.value.tagEditorPlayer && isRoute('tageditor')) ||
            isRoute('quicktag')
          "
        ></PlayerBar>
      </q-footer>
    ...
    <q-dialog v-model="sizeDialog" persistent>
      <q-card>
        ...
        <q-card-section>
          One Tagger requires atleast 1200x550 window size. Please resize to
          continue.
        </q-card-section>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="$1t.folderBrowser.value.open">
      <FolderBrowser
        v-if="$1t.folderBrowser.value.open"
        :base="$1t.folderBrowser.value.basePath"
      ></FolderBrowser>
    </q-dialog>
  </div>
</template>
...
const update = ref<
  undefined | { url: string; version: string; changelog: string }
>(undefined);
...
function setWaveformWidth() {
  document.documentElement.style.setProperty(
    "--waveform-wave",
    Math.min(Math.round(20 + (window.innerWidth - 1200) / 10), 70).toString(),
  );
}
