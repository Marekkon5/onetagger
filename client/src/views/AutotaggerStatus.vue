<template>
  <div class="text-center">
    <div class="text-subtitle1 text-bold text-primary q-mt-md">
      TAGGING STATUS
    </div>

    <!-- Post tagging actions -->
    <div
      v-if="$1t.taggerStatus.value.done && $1t.taggerStatus.value.data"
      class="row justify-center q-my-md"
    >
      <q-btn color="primary" class="q-mx-md text-black" @click="goQT(false)"
        >Open failed in QuickTag</q-btn
      >
      <q-btn color="primary" class="q-mx-md text-black" @click="goQT(true)"
        >Open successful in QuickTag</q-btn
      >
    </div>
            ...
            <div class="row">
              <div class="col q-mt-sm q-pt-xs text-left q-pl-md">
                <q-btn
                  icon="mdi-check"
                  round
                  :color="filter == 'ok' ? 'primary' : 'green'"
                  class="text-black"
                  @click="toggleFilter('ok')"
                >
                  <q-tooltip> Total amount found </q-tooltip>
                </q-btn>
              </div>
              <div class="col q-my-sm text-right q-pr-md">
                <div class="text-subtitle2 text-grey-6">Matched</div>
                <div class="text-subtitle1 monospace text-weight-bold">
                  {{ countStatus("ok") }}
                </div>
              </div>
            </div>
            ...
            <div class="row">
              <div class="col q-mt-sm q-pt-xs text-left q-pl-md">
                <q-btn
                  ...
                  round
                  :color="filter == 'error' ? 'primary' : 'red'"
                  class="text-black"
                  @click="toggleFilter('error')"
                >
                  <q-tooltip> Total amount not found </q-tooltip>
                </q-btn>
              </div>
              <div class="col q-my-sm text-right q-pr-md">
            ...
            <div class="row">
              <div class="col q-mt-sm q-pt-xs text-left q-pl-md">
                <q-btn
                  ...
                  round
                  :color="filter == 'skipped' ? 'primary' : 'yellow'"
                  class="text-black"
                  @click="toggleFilter('skipped')"
                >
                  <q-tooltip>
                    Total amount skipped due missing tags, corruption, or Shazam
            ...
            <div class="row">
              <div class="col q-mt-sm q-pt-xs text-left q-pl-md">
                <q-btn
                  ...
                  class="text-black"
                >
                  <q-tooltip> Total amount of files to process </q-tooltip>
                </q-btn>
              </div>
              <div class="col q-my-sm text-right q-pr-md">
                <div class="text-subtitle2 text-grey-6">Total</div>
                <div class="text-subtitle1 monospace text-weight-bold">
                  {{ $1t.taggerStatus.value.total }}
                </div>
              </div>
            </div>
          </q-card>
            ...
            <div class="row">
              <div class="col q-mt-sm q-pt-xs text-left q-pl-md">
                <q-btn
                  ...
                  class="text-black"
                >
                  <q-tooltip> Total amount of elapsed time </q-tooltip>
                </q-btn>
              </div>
              <div class="col q-my-sm text-right q-pr-md">
    ...
    <q-list class="list text-left bg-dark q-py-sm">
      <q-virtual-scroll
        :items="statuses"
        :class="{
          'status-list': !$1t.taggerStatus.value.done,
          'status-list-done': $1t.taggerStatus.value.done,
        }"
      >
            ...
            <q-item-section>
              <q-item-label overline>
                <span v-for="(i, index) in item">
                  <span
                    v-if="$1t.taggerStatus.value.type != 'audioFeatures'"
                    class="selectable text-white"
                    >{{ platformText(i.platform) }}</span
                  >
                  <span
                    v-if="$1t.taggerStatus.value.type == 'audioFeatures'"
                    class="selectable text-white"
                  ...
                  <q-icon
                    ...
                    :name="statusIcon(i.status.status)"
                    :color="statusColor(i.status.status)"
                  >
                    ...
                    <q-tooltip v-if="i.status.status == 'ok'">
                      ...
                      <span v-if="i.status.reason"
                        >, Reason: {{ i.status.reason }}</span
                      >
                    </q-tooltip>
                  </q-icon>
              ...
              <span class="selectable text-grey-5">{{
                item[0].status.path
              }}</span>
            </q-item-section>
          </q-item>
...
function countStatus(status: any) {
  return $1t.taggerStatus.value.statuses.reduce(
    (a, c) => (getStatus(c) == status ? a + 1 : a),
    0,
  );
}
...
function goQT(successful: boolean) {
  if (successful)
    $1t.settings.value.path = $1t.taggerStatus.value.data.successFile;
  if (!successful)
    $1t.settings.value.path = $1t.taggerStatus.value.data.failedFile;
  $router.push("/quicktag");
}

const statuses = computed(() => {
  if (!filter.value) return $1t.taggerStatus.value.statuses;
  return $1t.taggerStatus.value.statuses.filter(
    (s) => getStatus(s) == filter.value,
  );
});
...
onMounted(() => {
  ...
  timeInterval = setInterval(() => {
    ...
    time.value = `${Math.floor(s / 60)}:${Math.round(s % 60)
      .toString()
      .padStart(2, "0")}`;
  }, 400);
  // Done callback
