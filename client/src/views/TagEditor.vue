  ...
  <div class="full-height">
    <div class="row full-height">
      ...
      <div
        ...
        :class="{
          'col-4': !$1t.settings.value.tagEditorDouble,
          'col-3': $1t.settings.value.tagEditorDouble,
        }"
        style="max-height: 100%; overflow-y: scroll"
      >
        <div
          class="text-weight-bold text-subtitle2 clickable path-display"
          @click="browse"
        >
          <div class="row inline">
            <span style="direction: ltr" class="text-primary monospace">{{
              path
            }}</span>
          </div>
        </div>
        <div class="q-mt-sm">
          ...
          <q-input
            ...
            label="Filter"
            class="q-mb-sm"
            @update:model-value="(v: any) => applyFilter(v as string)"
            v-model="filter"
          ></q-input>
          ...
          >
            <template #item="{ element: file }">
              <div
                ...
                @click="
                  file.dir || file.playlist
                    ? loadFiles(file.filename)
                    : loadFile(file.path)
                "
                :class="{
                  'text-primary': isSelected(file.path),
                  'text-grey-4': !isSelected(file.path),
                }"
              >
                <q-icon
                  size="xs"
      ...
      >
        ...
        <div class="row justify-between">
          <div class="text-weight-bold text-subtitle2 text-primary q-pb-sm">
            Your list
          </div>
          <div>
            <q-btn
          ...
          <template #item="{ element: f }">
            <div class="row">
              <div
                ...
                :class="{
                  'text-primary': isSelected(f),
                  'text-grey-4': !isSelected(f),
                }"
              >
      ...
      <div
        :class="{
          'col-8': !$1t.settings.value.tagEditorDouble,
          'col-6': $1t.settings.value.tagEditorDouble,
        }"
        style="max-height: 100%; overflow-y: scroll"
      >
        <div
          v-if="!file"
          class="justify-center items-center content-center row full-height"
        >
          <div
            class="col-12 text-subtitle2 text-bold text-primary text-center q-my-sm"
          >
            NO FILE SELECTED
          </div>
          <br />
          <span class="text-center text-subtitle2 text-grey-6"
            >Tip:
            <span class="keybind-icon q-px-sm text-caption text-bold"
              >CLICK</span
            >
            the path to open a folder and select an audio file</span
          >
        </div>
        ...
        <div v-if="file" class="q-px-md">
          ...
          <div class="q-mt-md">
            <div
              v-for="(tag, i) in Object.keys(file.tags)"
              :key="i"
              class="row q-my-sm"
            >
              <div
                class="col-3 text-body2 text-uppercase text-primary text-weight-medium q-mt-sm q-pr-xs"
                style="text-overflow: ellipsis; overflow: hidden"
              >
                <span v-if="ABSTRACTIONS[tag]"
                  ><span class="text-uppercase text-primary text-weight-medium"
                    >{{ ABSTRACTIONS[tag] }} </span
                  ><span class="text-grey-4 monospace text-caption"
                    > {{ tag }}</span
                  ></span
                >
          ...
          <q-separator
            class="q-mx-auto"
            :style="'max-width: 513px; margin-top: 40px;'"
            inset
            color="dark"
          />
          ...
          <div class="row q-mt-lg" style="margin-top: 40px">
            <div
              class="col-3 q-pt-sm text-weight-medium text-grey-4 text-body2"
            >
              Add new text tag
            </div>
            <TagField
              ...
              dense
              :format="tagFormat!"
              @update:model-value="newTag = $event"
            ></TagField>
            <div class="col-1 q-pl-md q-pt-xs">
          ...
          <q-separator
            class="q-mx-auto"
            :style="'max-width: 513px; margin-top: 20px; margin-bottom: 25px;'"
            inset
            color="dark"
          />
          ...
          <div v-if="file.id3">
            ...
            <div class="text-uppercase text-primary text-weight-medium">
              Comments
              <span class="text-grey-4 monospace text-caption q-pl-xs"
                >COMM</span
              >
            ...
            <div>
              <div
                v-for="(comment, i) in file.id3.comments"
                :key="'comm' + i"
                class="row q-py-sm"
              >
                <q-input
            ...
            <div>
              <div
                v-for="(lyric, i) in file.id3.unsync_lyrics"
                :key="'uslt' + i"
                class="q-py-sm"
              >
                <div class="row">
            ...
            <q-separator
              class="q-mx-auto"
              :style="'max-width: 513px; margin-top: 32px; margin-bottom: 25px;'"
              inset
              color="dark"
            />
    ...
    <ManualTag
      :path="manualTagPath"
      @exit="
        loadFile(manualTagPath!);
        manualTagPath = undefined;
      "
    ></ManualTag>
  </div>
</template>
...
function loadFile(path: string) {
  ...
  if ($1t.settings.value.tagEditorPlayer) $1t.player.value.loadTrack(path);
}
...
function applyFilter(v: string) {
  ...
  files.value = originalFiles.value.filter((f) =>
    f.filename.toLowerCase().includes(filter.value),
  );
}
  ...
  if (e.added) {
    if (e.added.element.dir || e.added.element.playlist) {
      $1t.send("tagEditorFolder", {
        path: path.value,
        subdir: e.added.element.filename,
        recursive: true,
      });
      // Don't copy
      ...
      else customList.value.splice(e.added.newIndex, 1);
    }
  }
  // Read again
  if (e.removed) {
...
function addNewTag() {
  ...
  let i = changes.value.findIndex(
    (c) => c.type == "remove" && c.tag == newTag.value,
  );
...
function addAlbumArt(data: any) {
  ...
  changes.value = changes.value.filter(
    (c) => c.type != "addPictureBase64" || c.kind != data.kind,
  );
...
function removeArt(i: number) {
  ...
  let index = changes.value.findIndex(
    (c) => c.type == "addPictureBase64" && c.kind == kind,
  );
  if (index != -1) {
...
function id3POPMChange() {
  ...
  if (file.value.id3.popularimeter) {
    file.value.id3.popularimeter.counter = parseInt(
      file.value.id3.popularimeter.counter.toString(),
    );
    changes.value.push({
  ...
  switch (e.action) {
    ...
    case "tagEditorFolder":
      if (e.recursive) {
        ...
        let files = customList.value.concat(
          e.files
            .sort((a: any, b: any) => {
              return a.filename
                .toLowerCase()
...
const tagFormat = computed(() => {
  ...
  if (file.value.format == "flac" || file.value.format == "ogg")
    return "vorbis";
...
const albumArtTypes = computed(() => {
  ...
  return types.filter((t) =>
    file.value.images.find((i: any) => i.kind == t) ? false : true,
  );
});
