<template>
  <div>
    <div class="q-mt-xs q-mb-sm text-grey-5 row">
      ...
      <!-- Sort -->
      <div
        class="col-9 row q-pb-sm text-caption text-weight-medium bg-darker text-grey-6 text-capitalize justify-between clickable"
      ...
      >
        <span
          ...
          class="row q-pl-xs"
        >
          <div :class="{ 'text-grey-4 clickable': sortOption == option }">
            {{ option }}
          </div>
      ...
      <div class="col-1 clickable q-mr-md text-right">
        <div class="q-pr-xs">
          <q-btn
            :icon="
              $1t.settings.value.quickTag.thinTracks
                ? 'mdi-view-list'
    ...
    <div class="q-mx-lg text-grey-7 q-my-xs text-caption text-center">
      Loaded files:
      <span class="monospace text-bold">{{
        $1t.quickTag.value.tracks.length
      }}</span>
      <span class="q-ml-md">Filtered: </span
      ><span class="monospace text-bold">{{ tracks.length }}</span>
      <span v-if="$1t.quickTag.value.failed.length != 0"
        ><span class="q-ml-md">Failed to load: </span>
        <span class="monospace text-bold" @click="failedDialog = true"
          >{{ $1t.quickTag.value.failed.length }}
          <span class="text-weight-medium show-link cursor-pointer"
            >Show details</span
          >
        </span>
      </span>
      ...
      <span
        class="q-ml-md text-caption cursor-pointer"
        :style="'margin-left: 13px'"
        v-if="$1t.quickTag.value.isLimited()"
        @click="$1t.loadQuickTag(undefined, false)"
      >
        Loading was capped to
        <span class="text-caption monospace text-bold">500</span> tracks!
        <span class="q-ml-xs text-weight-medium show-link cursor-pointer"
          >Show all</span
        >
      </span>
    </div>
    ...
    <!-- Tracks -->
    <div
      class="tracklist qt-full-height"
      v-if="$1t.quickTag.value.tracks.length > 0"
      ref="tracklist"
      :class="{ 'qt-height': $1t.quickTag.value.track }"
      @scroll="onScroll"
    >
      <!-- Tracklist -->
      <div
      ...
      >
        <q-intersection
          style="height: 116px"
          @click.native="(e: MouseEvent) => trackClick(item, e)"
          once
        >
          <QuickTagTile
            :track="item"
            :no-art-cache="noArtCacheList.includes(item.path)"
          ></QuickTagTile>
          <QuickTagContextMenu
    ...
    <div
      v-if="$1t.quickTag.value.tracks.length == 0"
      class="qtbg-container qt-full-height"
    >
      <!-- Loading -->
      ...
      <div @click="selectFolder" v-if="!$1t.lock.value.locked">
        <div class="text-center text-subtitle2 text-bold text-primary q-my-sm">
          NO FOLDER SELECTED
        </div>
        <div class="text-center text-subtitle2 text-grey-6">
          <span class="keybind-icon q-px-sm text-caption text-bold">CLICK</span>
          here to select folder
        </div>

        <div class="q-pa-lg q-mt-lg">
          <div
            class="row q-py-xs text-caption text-grey-6 text-weight-medium text-right"
          >
          ...
          <div
            class="row text-body q-py-xs text-caption text-grey-6 text-weight-medium text-right"
          >
            <div class="col">Seek back / forwards:</div>
            <div class="col text-body text-weight-bold text-grey-6 text-left">
              <q-icon
                name="mdi-chevron-left"
                class="keybind-icon q-mr-xs"
              ></q-icon>
              / <q-icon name="mdi-chevron-right" class="keybind-icon"></q-icon>
            </div>
          </div>
          <div
            class="row text-body q-py-xs text-caption text-grey-6 text-weight-medium text-right"
          >
            <div class="col">Switch Track:</div>
            <div class="col text-body text-weight-bold text-grey-6 text-left">
              <q-icon
                name="mdi-chevron-up"
                class="keybind-icon q-mr-xs"
              ></q-icon>
              / <q-icon name="mdi-chevron-down" class="keybind-icon"></q-icon>
            </div>
          </div>
          <div
            class="row text-body q-py-xs text-caption text-grey-6 text-weight-medium text-right"
          >
            <div class="col">Select multiple:</div>
            <div class="col text-body text-weight-bold text-grey-6 text-left">
              <q-icon
                name="mdi-apple-keyboard-control"
                class="keybind-icon q-mr-xs"
              ></q-icon>
              + <span class="keybind-icon q-px-sm">CLICK</span>
            </div>
          </div>
          <div
            class="row text-body q-py-xs text-caption text-grey-6 text-weight-medium text-right"
          >
            ...
            <div class="col text-body text-weight-bold text-grey-6 text-left">
              <q-icon
                ...
                class="keybind-icon q-mr-xs"
              ></q-icon>
              + <span class="keybind-icon q-px-sm">S</span>
            </div>
          </div>
          <div
            class="row text-body q-py-xs text-caption text-grey-6 text-weight-medium text-right"
          >
            ...
            <div class="col text-body text-weight-bold text-grey-6 text-left">
              <q-icon
                ...
                class="keybind-icon q-mr-xs"
              ></q-icon>
              + <span class="keybind-icon q-px-sm">DEL</span>
            </div>
          </div>
          <div
            class="row text-body q-py-xs text-caption text-grey-6 text-weight-medium text-right"
          ...
          <div
            class="row text-body q-py-xs text-caption text-grey-6 text-weight-medium text-right"
          >
            <div class="col">Context Menu:</div>
            <div class="col text-body text-weight-bold text-grey-6 text-left">
              <span class="keybind-icon q-pl-sm">RIGHT</span
              ><span class="keybind-icon q-px-sm">CLICK</span>
            </div>
          </div>
        </div>
      ...
      <q-card>
        ...
        <q-card-section>
          <div class="text-grey-3">
            Warning, there are unsaved changes on this track, do you want to
            save them?
          </div>
        </q-card-section>
        <q-card-actions align="right">
          <q-btn color="red" flat text @click="saveDialogCallback(false)"
            >Discard</q-btn
          >
          <q-btn
            color="primary"
      ...
      <q-card v-if="$1t.quickTag.value.track">
        ...
        <q-card-section>
          <q-input
            ...
            @update:model-value="
              (d: any) => $1t.quickTag.value.track!.setNote(d)
            "
    ...
    <q-dialog v-model="failedDialog">
      <q-card class="q-pa-md">
        <q-card-section>
          <div class="text-subtitle2 text-bold text-center text-red">
            FAILED TO LOAD
          </div>
        </q-card-section>
        <q-card-section>
          <div>
            <div v-for="failed in $1t.quickTag.value.failed" class="q-my-sm">
              <div class="text-subtitle3 text-grey-4 monospace">
                {{ failed.path }}
              </div>
        ...
        <q-card-section horizontal>
          <q-space></q-space>
          <q-btn flat color="primary" @click="failedDialog = false"
            >Close</q-btn
          >
        </q-card-section>
      </q-card>
...
function trackClick(track: QTTrack, event: MouseEvent) {
  ...
  if (
    event.ctrlKey ||
    event.metaKey ||
    ($1t.info.value.os == "macos" && event.altKey)
  ) {
...
function filterTracks() {
  let t = (() => {
    ...
    if (filter.value) {
      ...
      tracks = $1t.quickTag.value.tracks.filter(
        (t) =>
          t.title.toLowerCase().match(newFilter) ||
          t.path.toLowerCase().match(newFilter) ||
          t.artists.filter((a: any) => a.toLowerCase().match(newFilter))
            .length > 0 ||
          (t.mood ?? "").toLowerCase().match(newFilter) ||
          t
            .getAllCustom()
            .some((i: CustomTagInfo) =>
              i.value.toLowerCase().match(newFilter),
            ) ||
...
function findIndex(highest: boolean = true) {
  ...
  for (let i = 0; i < $1t.quickTag.value.track.tracks.length; i++) {
    let index = tracks.value.findIndex(
      (t) => t.path == $1t.quickTag.value.track.tracks[i].path,
    );
...
function scrollToIndex(index: number) {
  if ($1t.settings.value.quickTag.thinTracks) {
    setVerticalScrollPosition(
      tracklist.value!,
      index * 33 - (tracklist.value!.clientHeight / 68) * 34,
      250,
    );
    return;
  }
  ...
  $1t.onQuickTagEvent = (action, data) => {
    switch (action) {
      ...
      case "addTrack":
        ...
        if (
          $1t.quickTag.value.track.tracks.length > 1 &&
      ...
      case "quickTagLoad":
        if (
          ...
          $1t.quickTag.value.tracks.length == 0 ||
          $1t.lock.value.locked
        )
          return;
        // Reload last opened track track
        setTimeout(() => {
          $1t.loadQTTrack(
            $1t.quickTag.value.tracks[$1t.settings.value.quickTag.trackIndex],
          );
          $1t.settings.value.quickTag.trackIndex = -1;
        }, 50);
      ...
      case "onDeleteTrack":
        ...
        $q.dialog({
          ...
          $1t.send("deleteFiles", {
            paths: $1t.quickTag.value.track.tracks.map((t) => t.path),
          });
          setTimeout(() => {
            $1t.quickTag.value.track.removeAll();
            $1t.loadQuickTag();
...
onUnmounted(() => {
  ...
  if ($1t.quickTag.value.track.tracks.length == 1) {
    $1t.settings.value.quickTag.trackIndex =
      $1t.quickTag.value.tracks.findIndex(
        (t) => $1t.quickTag.value.track.tracks[0].path == t.path,
      );
  } else {
    $1t.settings.value.quickTag.trackIndex = -1;
  }
