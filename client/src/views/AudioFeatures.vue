  ...
  <div class="text-center af-wrapper">
    ...
    <div v-if="!$1t.spotify.value.authorized" class="af-content">
      ...
      <div class="q-mt-xl text-subtitle2 text-grey-6" style="line-height: 24px">
        Automatically tag your local audio files, with so called audio features
        by Spotify, based on
        <q-badge outline color="primary"
          ><span class="text-uppercase text-grey-3">ISRC</span></q-badge
        >
        tag or exact match.<br />
        More info?<span class="q-px-sm text-caption text-bold click-highlight"
          >CLICK</span
        >
    ...
    <div v-if="$1t.spotify.value.authorized" class="af-content">
      ...
      <div class="text-subtitle2 q-mb-md text-grey-6">
        Drag & drop folder, copy/paste path directly or<span
          class="q-px-sm text-caption text-bold click-highlight"
          >CLICK</span
        >
        the
      ...
      >
        ...
        <div class="col-1">
          <q-icon
            name="mdi-help-circle-outline text-grey-6"
            class="playlist-tooltip q-mx-sm q-mt-xl q-pt-sm"
          >
      ...
      <div class="text-subtitle2 text-grey-6">
        Converts most prominent audio features value 0-100 to a description -
        based on threshold - and writes to selected tag frame
      </div>
      <div class="text-subtitle2 q-mt-xs q-mb-md text-grey-4">
        e.g. #dance-high, #energy-med, #vocal-low, #positive, #popular
      </div>

      <TagFields
        style="max-width: 550px; margin: auto"
        v-model="config.mainTag"
      ></TagFields>
      ...
      <div class="q-px-xl">
        ...
        <div v-for="(_, key, i) in config.properties" :key="'P' + i">
          <div class="row">
            ...
            <div class="col-1">
              <q-checkbox
                v-model="config.properties[key].enabled"
                class="checkbox"
              ></q-checkbox>
            </div>
            <!-- Title -->
      ...
      <div class="column flex-center">
        <q-toggle
          ...
          style="width: 200px"
          label="Write OneTagger meta tag"
          left-label
          v-model="config.metaTag"
        ></q-toggle>
        <q-toggle
          class="justify-between"
          style="width: 200px"
        ...
        <q-toggle
          ...
          left-label
          v-model="config.includeSubfolders"
        ></q-toggle>
      </div>
    ...
    <q-dialog v-model="cliDialog">
      <CliDialog
        :config="config"
        command="audiofeatures"
        :extra="`--client-id ${$1t.spotify.value.clientId} --client-secret ${$1t.spotify.value.clientSecret}`"
      ></CliDialog>
    </q-dialog>
  </div>
</template>
...
onMounted(() => {
  ...
  if ($1t.settings.value.audioFeatures.config) {
    config.value = AudioFeaturesConfig.fromJson(
