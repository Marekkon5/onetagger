  ...
  <div class="text-center">
    <div class="q-py-lg" v-if="!$1t.lock.value.locked">
      <div style="max-width: 800px; margin: auto">
        ...
        <div class="text-subtitle2 q-mb-md text-grey-6">
          Drag & drop folder, copy/paste path directly or<span
            class="keybind-icon text-caption text-bold"
            >CLICK</span
          >
          the <q-icon name="mdi-open-in-app"></q-icon> icon to browse
        </div>
        ...
        <div
          class="q-pt-lg row justify-center input"
          style="max-width: 725px; margin: auto"
        >
          <q-input
            filled
            class="col-10"
            label="Output folder (leave empty for same as input)"
        ...
        <q-separator
          class="q-mx-auto"
          :style="'max-width: 513px; margin-top: 41px;'"
          inset
          color="dark"
        />
        ...
        <div class="text-subtitle2 text-grey-6">
          Enter dynamic content and/or static content. More info?<span
        ...
        <div style="margin-top: -25px">
          ...
          <div class="template-text">
            ...
            <span v-if="!config.template" class="template-input-placeholder"
              >Filename template</span
            >
          </div>
          <input
          ...
          <div class="suggestions-box" :style="suggestionsStyle">
            ...
            <div style="width: 40%">
              ...
              >
                ...
                <q-icon
                  name="mdi-variable"
                  class="q-mb-xs"
                  v-if="suggestion.kind == 'variable'"
                ></q-icon>
                <q-icon
                  name="mdi-information-outline"
                  class="q-mb-xs"
                  v-if="suggestion.kind == 'property'"
                ></q-icon>
                <q-icon
                  name="mdi-function"
                  class="q-mb-xs"
                ...
                <span
                  class="q-ml-sm"
                  :class="{ 'text-primary': i == suggestionIndex }"
                >
                  <RenamerTokenName
                    :token="suggestion"
                    :params="false"
            ...
            <div style="width: 60%">
              <div v-if="suggestions[suggestionIndex]">
                <!-- Function info -->
                <div
                  v-if="suggestions[suggestionIndex].kind == 'function'"
                  class="q-mb-sm suggestion-help-function"
                >
                  <RenamerTokenName
                    :token="suggestions[suggestionIndex]"
                  ></RenamerTokenName>
                  <br />
                </div>
      ...
      <div class="full-width">
        <div
          class="q-mt-md q-mb-sm text-subtitle2 text-bold text-primary custom-margin"
        >
          PREVIEW
        </div>
        <div v-for="(file, i) in preview" :key="'prev' + i">
          <div class="text-caption monospace text-grey-5 q-my-md">
            {{ file[1] }}
          </div>
        </div>
      </div>
      ...
      <q-separator
        class="q-mx-auto"
        :style="'max-width: 513px; margin-top: 34px;'"
        inset
        color="dark"
      />

      <div
        class="text-subtitle2 text-bold text-primary custom-margin"
      ...
      <div class="column flex-center">
        ...
        <q-toggle
          ...
          v-model="config.subfolders"
        ></q-toggle>
        <q-toggle
          left-label
          class="justify-between"
          style="width: 240px"
          label="Keep original subfolders"
          v-model="config.keepSubfolders"
        ></q-toggle>
      </div>
    ...
    <div>
      <span
        style="visibility: hidden; font-size: 16px"
        class="monospace"
        ref="textWidthRef"
...
function onKeyDown(e: KeyboardEvent) {
  ...
  if (e.key == "Enter") {
    if (suggestions.value[suggestionIndex.value]) {
      // Fill suggestion
      let text = suggestions.value[suggestionIndex.value].name.substring(
  ...
  // Don't close again
  if (
    templateInputElem.value?.selectionStart ==
    templateInputElem.value?.selectionEnd
  ) {
    if (e.key == ")") {
...
function injectTemplate(index: number, text: string) {
  templateInputElem.value!.value =
    templateInputElem.value!.value.substring(0, index) +
    text +
    templateInputElem.value!.value.substring(index);
...
function start(force = false) {
  ...
  if (!force) {
    $q.dialog({
      title: "Warning",
      message:
        "Many DJ apps store cue points and other metadata based on the original file name. When renamed, this information will be lost and you will have to reimport these files.",
      html: true,
      ok: {
        color: "primary",
        label: "Start",
...
onMounted(() => {
  $1t.onRenamerEvent = (json: any) => {
    switch (json.action) {
      // Browse folder
      case "browse":
        if (json.context == "rnOutput") config.value.outDir = json.path;
        else config.value.path = json.path;
      ...
      case "renamerDone":
        ...
        $q.dialog({
          ...
          $1t.send("openFolder", {
            path: config.value.outDir ?? config.value.path,
          });
        });
        break;
      // Suggestions
  ...
  if ($1t.settings.value.renamer) {
    ...
    if (config.value.template) {
      $1t.send("renamerSyntaxHighlight", { template: config.value.template });
      // @ts-ignore
      document.getElementsByClassName("template-input")[0].value =
...
const cursorStyle = computed(
  () => `margin-left: ${12 + cursor.value * charWidth.value}px`,
);
