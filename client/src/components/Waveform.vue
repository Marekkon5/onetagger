<template>
  <div>
    <div class="row items-center justify-between q-pr-lg">
      ...
      >
        <span
          v-for="(wave, i) in $1t.player.value.waveform"
          class="wavefont"
          :key="wave + i"
        >
          {{ waveChar(wave) }}
      ...
      <span class="q-pl-sm monospace q-pb-xs">{{
...
function waveChar(wave: number) {
  return String.fromCharCode(
    0x100 + Math.min(Math.round((wave + 0.05) * 100), 100),
  );
}
...
const waveformBackground = computed(() => {
  if (!hover.value) {
    ...
    let audioPos =
      ($1t.player.value.position / $1t.player.value.duration) * 100.0;
