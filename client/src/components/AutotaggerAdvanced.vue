<template>
  <div class="text-left q-pt-xs" style="max-width: 450px; margin: auto">
    <q-list>
      ...
      <div v-if="!$1t.config.value.overwrite" class="q-pb-md">
        <q-select
          ...
          :model-value="
            SUPPORTED_TAGS.filter((t) =>
              $1t.config.value.overwriteTags.includes(t.tag),
            )
          "
          @update:model-value="
            (v) => ($1t.config.value.overwriteTags = v.map((v: any) => v.tag))
          "
      ...
      ></AdvancedSettingsToggle>
      <div
        class="row justify-center items-center q-px-md"
        v-if="$1t.config.value.albumTagging"
      >
        <div class="q-pb-lg">
    ...
    <q-separator
      class="q-mx-auto q-mt-md custom-sep-advanced"
      :style="'margin-top: 21px;'"
      inset
      color="dark"
    />
    <div
      class="text-subtitle2 text-center text-bold text-primary q-mt-lg"
      :style="'margin-top: 35px;'"
    >
      GENRE / STYLE / SUBGENRE
      <div class="text-subtitle2 q-mb-md text-grey-6">
        Fetch all <span class="text-grey-4">(default)</span>, if it should merge
        them, or write elsewhere <br /><span class="text-grey-4"
          >(supported platforms only)</span
        >
      </div>
      ...
      <div v-if="$1t.config.value.stylesOptions == 'customTag'">
        <TagFields
          v-model="$1t.config.value.stylesCustomTag"
          class="q-mb-md"
        ></TagFields>
      </div>
    </div>
    ...
    <q-separator
      class="q-mx-auto q-mb-xl custom-sep-advanced"
      :style="'margin-top: 21px;'"
      inset
      color="dark"
    />
    ...
    <div class="row justify-center q-mt-lg" style="max-width: 550px">
      <q-slider
        ...
        :label-value="
          'Strictness: ' + Math.round($1t.config.value.strictness * 100) + '%'
        "
      >
      </q-slider>
      <q-icon
    ...
    <div class="row justify-center" style="max-width: 550px">
      ...
      >
        <q-tooltip
          >The higher, the faster, but uses more bandwidth, and not supported by
          all platforms</q-tooltip
        >
      </q-icon>
    </div>
    ...
    <div
      ...
      v-if="$1t.config.value.matchDuration"
    >
      <q-slider
        ...
        label-text-color="black"
        :label-value="
          'Max difference: ' + $1t.config.value.maxDurationDifference + 's'
        "
      ></q-slider>
    </div>
    <br />
    ...
    />
    <div
      class="text-subtitle2 text-center text-bold text-primary q-mt-lg q-mb-sm"
      :style="'margin-top: 35px;'"
    >
      SEPARATORS
    </div>
    ...
    <div
      class="text-subtitle2 text-center text-bold text-primary"
    ...
    <q-separator
      class="q-mx-auto q-mb-lg custom-sep-advanced"
    ...
    >
      ...
      >
        <q-tooltip>
          $success and $failed will be substituted with paths to files
          containing paths of affected tracks
        </q-tooltip>
      </q-icon>
    </div>
    <q-input
      label="$success, $failed will be substituted"
      filled
      class="row input q-py-sm justify-center"
      style="max-width: 526px; margin: auto"
      v-model="$1t.config.value.postCommand"
    ></q-input>
    <br />
...
const stylesOptions = [
  ...
  "Merge to Genre tag",
  "Merge to Style tag",
  "Write Style to Genre tag",
  "Write Genre to Style tag",
...
function updateStyleOption() {
  $1t.config.value.stylesOptions =
    values[stylesOptions.indexOf(stylesOption.value)];
}

onMounted(() => {
  stylesOption.value =
    stylesOptions[values.indexOf($1t.config.value.stylesOptions)];
