<template>
        ...
        <q-card flat class="card q-ma-md">
          <q-card-section horizontal class="row justify-between">
            <q-card-section>
              <div class="row">
                <q-checkbox
                  :model-value="isEnabled(platform.platform.id)"
                  class="cb"
                  @update:model-value="update(platform.platform.id)"
                ></q-checkbox>
                <div class="text-h6 q-mt-xs">
                  ...
                  <span>
                    <span class="q-pl-xs text-grey-7">
                      <q-icon
                        v-if="platform.platform.maxThreads == 1"
                      ...
                      ></q-icon>
                      <q-icon
                        v-if="platform.platform.maxThreads == 0"
                    ...
                    <q-tooltip>
                      <span v-if="platform.platform.maxThreads"
                        >This platform allows up to
                        {{ platform.platform.maxThreads }} concurrent
                        searches</span
                      >
                      <span v-else
              ...
              <div
                v-if="!dense"
                class="text-subtitle2 q-ml-sm text-left text-grey-6"
              >
                <span v-html="platform.platform.description"></span>
              </div>
              <div
                v-if="!platform.builtIn"
                class="text-grey-8 q-pl-sm text-bold monospace text-left"
  ...
  <div v-if="!dense" class="q-mt-md q-mb-xl">
    ...
    <div class="text-subtitle2 text-grey-6">
      One Tagger supports custom platforms written in Rust.<br />You can install
    ...
    <div class="row items-center justify-center q-mt-md">
      <q-btn
        ...
        push
        color="primary"
        :loading="platformsRepoButtonLoading"
        :disable="platformsRepoButtonLoading"
        class="rounded-borders q-px-sm q-mb-xs text-black text-weight-medium text-caption"
        @click="openPlatformsRepo()"
        >Platforms Repository</q-btn
      >
    </div>
    ...
    <div class="text-caption text-grey-6 text-center q-py-sm">
      <span
        @click="
          $1t.url(
            'https://github.com/Marekkon5/onetagger/blob/master/CUSTOM_PLATFORMS.md',
          )
        "
        class="clickable doc-link"
        >How to create a custom platform?<span class="q-ml-xs"
          ><q-icon name="mdi-open-in-new"></q-icon></span
      ></span>
    </div>
  </div>
</template>
...
function update(platform: string) {
  ...
  if (i == -1) $1t.config.value.platforms.push(platform);
  else $1t.config.value.platforms.splice(i, 1);
}
...
function syncPlatforms() {
  $1t.config.value.platforms = $1t.info.value.platforms
    .map((p) => p.platform.id)
    .filter((p) => $1t.config.value.platforms.includes(p));
}

/// Does the platform have lyrics
function hasLyrics(platform: AutotaggerPlatform) {
  return (
    platform.supportedTags.includes(SupportedTag.UnsyncedLyrics) ||
    platform.supportedTags.includes(SupportedTag.SyncedLyrics)
  );
}
