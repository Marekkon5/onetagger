<template>
  <div class="q-pa-md">
    ...
    <div
      ...
      v-if="$1t.quickTag.value.track.tracks.length == 1"
    >
      <q-btn
        outline
        color="primary"
        class="text-caption"
        @click="
          $1t.onQuickTagEvent('onManualTag', {
            path: $1t.quickTag.value.track.tracks[0].path,
          })
        "
        >MANUAL TAG</q-btn
      >
...
function addNewTag() {
  if (newTagValue.value) {
    $1t.settings.value.quickTag.custom[newTag.value].values.push({
      val: newTagValue.value,
      keybind: undefined,
    });
    $1t.saveSettings();
  }
