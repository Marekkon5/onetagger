<template>
  <div class="text-center">
    ...
    <div v-for="(platform, i) in platforms" :key="'p' + i" class="q-mb-md">
      <div
        class="text-subtitle2 text-bold text-uppercase text-grey-4"
        style="margin-top: 1px; margin-bottom: -16px"
      >
        {{ platform.name }}
      </div>
      ...
      <!-- Custom options -->
      <div
        v-for="(option, j) in platform.customOptions.options"
        :key="i + 'o' + j"
        class="q-mt-lg"
      >
        <!-- Slider -->
        <div v-if="option.value.type == 'number'">
          <q-chip text-color="black" color="primary">
            {{ option.label }}:
            {{ $1t.config.value.custom[platform.id][option.id] }}
        ...
        <div v-if="option.value.type == 'button'">
          <div>
            <div
              class="text-subtitle2 text-grey-6 q-mb-md"
              v-if="option.tooltip"
              v-html="option.tooltip"
            ></div>
            <q-btn
              ...
              :label="option.label"
              @click="
                $1t.send('configCallback', {
                  platform: platform.id,
                  config: $1t.config.value.custom[platform.id],
                  id: option.id,
                })
              "
            ></q-btn>
      ...
      <q-separator
        class="q-mx-auto"
        :style="'max-width: 513px; margin-top: 40px; margin-bottom: 35px'"
        inset
        color="dark"
      />
    </div>
    ...
    <div v-if="spotify">
      ...
      <div v-if="$1t.spotify.value.authorized">
        <div class="q-mt-lg text-h7 text-primary">
  ...
  $1t.info.value.platforms
    .filter(
      (p) =>
        ...
        p.platform.customOptions.options.length > 0,
    )
    .map((p) => p.platform),
);
</script>
