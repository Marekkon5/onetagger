<template>
  ...
  >
    <q-card
      flat
      class="elevation-4"
      :class="{ 'bg-highlight': drag, 'playlist-drop-zone': !drag }"
      v-if="!tiny"
    >
      <q-card-section>
        <div
        ...
        >
          <span class="text-subtitle1 text-grey-4" v-if="!filename">
            <q-icon
              ...
              size="sm"
              class="q-pr-sm q-pb-xs text-grey-6"
            ></q-icon>
    ...
    <div v-if="tiny">
      <q-btn
        dense
        class="q-mr-sm text-body2 text-capitalize"
      ...
      >
        <q-tooltip>
          Drag & drop playlist here / click to remove it, or export playlist
          from selected / filtered tracks
        </q-tooltip>
      </q-btn>
    </div>
  </div>
