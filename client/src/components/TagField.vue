...
function removeHelper() {
  if (
    (format == "id3" || format == "mp4") &&
    value.value.match(/[©a-zA-Z0-9]{4} \(.*\)/)
  ) {
...
function onInputValue(v: string) {
  ...
  if (!v) error.value = "Shouldn't be empty!";
...
function onFilter(val: any, update: any) {
  update(() => {
    ...
    options = originalOptions.filter((v: string) =>
      v.toUpperCase().includes(n),
    );
  });
}
...
const label = computed(() => {
  switch (format) {
    case "id3":
      return "ID3 (MP3/AIFF/WAV)";
    case "vorbis":
      return "Vorbis (FLAC/OGG/OPUS)";
    case "mp4":
      return "MP4/M4A";
  }
});
...
const color = computed(() => {
  if ((format == "id3" || format == "mp4") && (value.value ?? "").length != 4)
    return "yellow";
  return "primary";
});
