<template>
  <div>
    <q-card
      flat
      class="q-mx-md qt-tile"
      :class="{ 'bg-onetagger-icon': selected }"
    >
      <div class="row qt-tile">
        ...
        <div class="row q-pt-md q-pl-md full-width full-height">
          ...
          <div class="col-4 q-pl-sm">
            <span
              class="text-subtitle2 text-grey-4 text-weight-medium text-no-wrap title-span qt-tile-main"
              >{{ track.title }}</span
            >
            <span
              class="text-subtitle2 text-grey-6 text-weight-medium text-no-wrap title-span"
              >{{ track.artists.join(", ") }}</span
            >
          </div>
          <!-- Details -->
          <div
            class="col-7 row text-center text-subtitle2 text-weight-medium items-center"
          >
            ...
            <div
              class="col-4 qt-tile-col text-grey-4 text-caption text-weight-bold"
            >
              <!-- Genres -->
              <div v-if="selected">
                <span
              ...
              <div class="text-grey-6 text-weight-medium monospace">
                {{ track.year }}
              </div>
            </div>
            <div
              class="mt-3 col-1 qt-tile-col text-caption text-grey-4 text-weight-medium"
            >
      ...
      <!-- Custom tags -->
      <div
        class="row q-mx-sm no-wrap overflow-hidden custom-tag-chips text-subtitle2"
      >
        <div
...
const selected = computed(() =>
  $1t.quickTag.value.track.isSelected(track.value),
);
const art = computed(
  () =>
    `${httpUrl()}/thumb?path=${encodeURIComponent(track.value.path)}${noArtCache.value ? "&_=" + Math.random().toString() : ""}`,
);
</script>
