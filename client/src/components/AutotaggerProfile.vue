...
function removeProfile() {
  let i = $1t.settings.value.autoTaggerProfiles.findIndex(
    (p) => p.name == $1t.settings.value.autoTaggerProfile,
  );
  if (i == -1) return;
...
function saveProfile() {
  $1t.settings.value.saveATProfile(
    $1t.settings.value.autoTaggerProfile,
    $1t.config.value,
  );
  $1t.saveSettings();
}
