<template>
  ...
  <q-card
    class="qt-tile-thin row items-center"
    :class="{ 'qt-tile-thin-selected': selected, 'qt-tile-thin-odd': odd }"
  >
    <div
      class="text-caption text-grey-4 text-weight-medium q-px-sm text-no-wrap ellipsis"
      style="width: 12vw"
    >
      {{ track.title }}
    </div>
    <div
      class="text-caption title-span text-grey-6 text-weight-medium q-px-sm text-no-wrap ellipsis"
      style="width: 12vw"
    >
      {{ track.artists.join(", ") }}
    </div>
    ...
    <!-- Genres -->
    <div
      class="text-grey-4 text-caption text-weight-bold text-no-wrap ellipsis"
      style="width: 10vw"
    >
      <span v-if="selected">
        <span
    ...
    <div class="text-no-wrap ellipsis row q-pl-sm" style="width: 132px">
      <div class="col-4">
        <div class="text-caption monospace text-grey-6 text-weight-medium">
          {{ track.year }}
        </div>
      </div>
      <div class="col-4">
        <div
          class="text-caption monospace text-grey-4 text-weight-medium"
          v-if="track.bpm"
        >
          {{ track.bpm }}
        </div>
