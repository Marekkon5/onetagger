<template>
  <div class="q-px-md">
    <!-- Path -->
    <div
      class="text-weight-bold clickable path-display q-my-md"
      v-if="!editPath"
    >
      <div class="row inline">
        <span style="direction: ltr">
          <span @click="browse" class="text-primary monospace q-pr-xs">{{
            path
          }}</span>
          <q-icon
            name="mdi-pencil"
      ...
      <div v-for="file in files" :key="file.filename">
        <div
          ...
          :class="{
            ...
            'text-grey-5': !isSelected(file.path),
          }"
        >
          <q-icon
            size="xs"
...
function applyFilter() {
  ...
  files.value = originalFiles.value.filter((f) =>
    f.filename.toLowerCase().includes(filter.value?.toLowerCase()),
  );
}
...
onMounted(() => {
  ...
  $1t.onQuickTagBrowserEvent = (json) => {
    switch (json.action) {
      ...
      case "pathUpdate":
        initial.value = true;
        $1t.send("quickTagFolder", {
          path: $1t.settings.value.path,
          subdir: "..",
        });
    }
  };
