  ...
  <div class="text-center">
    ...
    <div v-if="!manualTag">
      ...
      <div class="text-subtitle2 q-mb-md text-grey-6">
        ...
        the browse
        <span
          ><q-icon name="mdi-open-in-app" class="q-mb-xs"></q-icon> icon</span
        >
      </div>
      <div
      ...
      <q-separator
        class="q-mx-auto"
        :style="'max-width: 513px; margin-top: 8px;'"
        inset
        color="dark"
      />
    </div>
    ...
    <div
      class="text-subtitle2 q-mt-lg text-primary text-bold"
    ...
    <div class="q-pt-xs q-mb-md" :style="'max-width: 550px; margin: auto;'">
      <div
        class="row justify-between tags wrap"
        :class="{ 'q-ml-md': manualTag, 'q-ml-xl': !manualTag }"
      >
        <!-- All tags -->
        <div v-for="tag in SUPPORTED_TAGS">
          ...
          >
            <q-icon
              v-if="tag.tooltip"
              name="mdi-help-circle-outline text-grey-6"
              class="onetagger-tooltip q-mx-xs mt-n2"
            >
    ...
    <div class="row justify-center q-mb-xs">
      <q-btn
        ...
        dense
        color="primary"
        class="rounded-borders q-mx-sm q-px-sm q-mt-md text-black text-weight-medium text-caption"
        @click="toggleTags('enableAll')"
        >Enable All</q-btn
      >
      <q-btn
        push
...
function isSupported(tag: SupportedTag) {
  ...
  return (
    $1t.info.value.platforms.find(
      (p) =>
        $1t.config.value.platforms.includes(p.platform.id) &&
        p.supportedTags.includes(tag),
    ) != null
  );
}
...
function toggleTags(mode: string) {
  switch (mode) {
    ...
    case "disableAll":
      ...
      break;
    case "toggle":
      $1t.config.value.tags = SUPPORTED_TAGS.map((t) => t.tag).filter(
        (t) => !$1t.config.value.tags.includes(t),
      );
      break;
  }
}
