...
use onetagger_platforms::spotify::rspotify::model::{Id, TrackId};
use onetagger_platforms::spotify::{rspotify, Spotify};
use onetagger_tag::{AudioFileFormat, FrameName, Tag, TagSeparators};
...
use crate::{AudioFileInfoImpl, TaggingState, TaggingStatus, TaggingStatusWrap, STOP_TAGGING};
...
impl Default for AFProperties {
    fn default() -> Self {
        ...
        Self {
            acousticness: AFProperty {
                enabled: true,
                range: AFRange { min: 0, max: 90 },
                tag: FrameName {
                    id3: "1T_ACOUSTICNESS".to_string(),
                    vorbis: "1T_ACOUSTICNESS".to_string(),
                    mp4: "1T_ACOUSTICNESS".to_string(),
                },
            },
            danceability: AFProperty {
                enabled: true,
                range: AFRange { min: 20, max: 80 },
                tag: FrameName {
                    id3: "1T_DANCEABILITY".to_string(),
                    vorbis: "1T_DANCEABILITY".to_string(),
                    mp4: "1T_DANCEABILITY".to_string(),
                },
            },
            energy: AFProperty {
                enabled: true,
                range: AFRange { min: 20, max: 90 },
                tag: FrameName {
                    id3: "1T_ENERGY".to_string(),
                    vorbis: "1T_ENERGY".to_string(),
                    mp4: "1T_ENERGY".to_string(),
                },
            },
            instrumentalness: AFProperty {
                enabled: true,
                range: AFRange { min: 50, max: 90 },
                tag: FrameName {
                    id3: "1T_INSTRUMENTALNESS".to_string(),
                    vorbis: "1T_INSTRUMENTALNESS".to_string(),
                    mp4: "1T_INSTRUMENTALNESS".to_string(),
                },
            },
            liveness: AFProperty {
                enabled: true,
                range: AFRange { min: 0, max: 80 },
                tag: FrameName {
                    id3: "1T_LIVENESS".to_string(),
                    vorbis: "1T_LIVENESS".to_string(),
                    mp4: "1T_LIVENESS".to_string(),
                },
            },
            speechiness: AFProperty {
                enabled: true,
                range: AFRange { min: 0, max: 70 },
                tag: FrameName {
                    id3: "1T_SPEECHINESS".to_string(),
                    vorbis: "1T_SPEECHINESS".to_string(),
                    mp4: "1T_SPEECHINESS".to_string(),
                },
            },
            valence: AFProperty {
                enabled: true,
                range: AFRange { min: 15, max: 85 },
                tag: FrameName {
                    id3: "1T_VALENCE".to_string(),
                    vorbis: "1T_VALENCE".to_string(),
                    mp4: "1T_VALENCE".to_string(),
                },
            },
            popularity: AFProperty {
                enabled: true,
                range: AFRange { min: 0, max: 80 },
                tag: FrameName {
                    id3: "1T_POPULARITY".to_string(),
                    vorbis: "1T_POPULARITY".to_string(),
                    mp4: "1T_POPULARITY".to_string(),
                },
            },
        }
    }
...
impl AFProperties {
    ...
    pub fn merge_with_values(
        &self,
        features: &rspotify::model::audio::AudioFeatures,
        track: &FullTrack,
        format: AudioFileFormat,
    ) -> Vec<AFPropertyMerged> {
        vec![
            ...
            AFPropertyMerged::new(features.energy, &self.energy, &format).add_main_value(
                "#energy-low",
                "#energy-med",
                "#energy-high",
            ),
            ...
            AFPropertyMerged::new(features.liveness, &self.liveness, &format).add_main_value(
                "#recording",
                "",
                "#live",
            ),
            ...
            AFPropertyMerged::new(features.valence, &self.valence, &format).add_main_value(
                "#negative",
                "#balanced",
                "#positive",
            ),
...
impl AudioFeatures {
        ...
        thread::spawn(move || {
            for (i, file) in files.iter().enumerate() {
                    ...
                    if config.skip_tagged && info.tagged.af() {
                        ...
                        match AudioFeatures::find_features(&spotify, &info) {
                            Ok((features, full_track)) => {
                                // Write to file
                                match AudioFeatures::write_to_path(
                                    &file,
                                    &features,
                                    &full_track,
                                    &config,
                                    ...
                                    Err(e) => {
                                        ...
                                        status.message = Some(format!(
                                            "Audio features failed writing to tag: {}",
                                            e
                                        ));
                            ...
                            Err(e) => {
                                ...
                                status.message = Some(format!(
                                    "Audio features search track by ISRC error: {}",
                                    e
                                ));
                                status.status = TaggingState::Error;
                            }
                ...
                tx.send(TaggingStatusWrap::wrap(
                    "spotify", &status, 0, 1, i as i64, file_count,
                ))
                .ok();
            }
        });
    ...
    fn find_features(
        spotify: &Spotify,
        track: &AudioFileInfo,
    ) -> Result<(rspotify::model::audio::AudioFeatures, FullTrack), Error> {
        ...
        if track_id.is_none() {
            let q = format!(
                ...
                track
                    .artists
                    .first()
                    .ok_or(anyhow!("Track is missing artist"))?
                    .to_lowercase(),
                MatchingUtils::clean_title(track.title()?)
            );
        ...
        // Get features
        let features = spotify.audio_features(&TrackId::from_id(
            &track_id.ok_or(anyhow!("Invalid track / no match"))?,
        )?)?;
    ...
    fn write_to_path(
        ...
        for property in config
            .properties
            .merge_with_values(features, full_track, format.clone())
        {
        ...
        if config.meta_tag {
            ...
            tag.set_raw(
                "1T_TAGGEDDATE",
                vec![format!("{}_AF", time.format("%Y-%m-%d %H:%M:%S"))],
                true,
            );
        }
